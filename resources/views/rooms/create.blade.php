<x-app-layout>
    ...
    <form method="post" action="{{ route('rooms.store', $property) }}" class="space-y-6 bg-white p-6 rounded border">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><x-input-label value="Name" /><x-text-input name="name" class="w-full" required /></div>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('rooms.index', $property) }}" class="px-3 py-2 rounded border">Cancel</a>
        </div>
    </form>
</x-app-layout>
