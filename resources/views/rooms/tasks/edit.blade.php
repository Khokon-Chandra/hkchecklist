<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl">Edit Task — {{ $room->name }}</h2>
            <a class="text-sm underline text-gray-600 dark:text-gray-300"
               href="{{ route('rooms.tasks.index', $room) }}">← Back</a>
        </div>
    </x-slot>
    ...
    <x-card>
        ...
        <form method="post"
            ...
            <div class="flex gap-2">
                ...
                <x-button variant="secondary" href="{{ route('rooms.tasks.index', $room) }}">Cancel</x-button>
    ...
    <form id="detach-form"
          method="post"
          action="{{ route('rooms.tasks.detach', [$room, $task]) }}"
          class="hidden">
        @csrf
        @method('DELETE')
    </form>
