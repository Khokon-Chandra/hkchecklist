<x-app-layout>
    ...
    <x-card>
        {{-- MAIN UPDATE FORM (only this form wraps the fields + Save/Cancel) --}}
        <form method="post" action="{{ route('properties.tasks.update', [$property, $room, $task]) }}" class="space-y-6">
            @csrf
            @method('PUT')

            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {{-- Left: identify/switch + pivot fields --}}
                <div class="lg:col-span-2" x-data="taskAutocomplete({ suggestUrl: @js($suggestUrl) })" x-init="q = @js($task->name)">
                    ...
                    <div class="relative mt-1">
                        ...
                        <div x-cloak x-show="open"
                            ...
                            id="task-suggest">
                            <div x-show="loading" class="px-3 py-2 text-sm text-gray-500 dark:text-gray-400">Searching…
                            </div>
                    ...
                    <div class="mt-3 flex items-center gap-4">
                        <label class="inline-flex items-center gap-2 text-sm">
                            <input type="checkbox" name="visible_to_owner" value="1"
                                class="rounded border-gray-300 dark:border-gray-700" @checked((bool) old('visible_to_owner', $pivot->visible_to_owner))>
                            Owner can view
                        </label>
                        <label class="inline-flex items-center gap-2 text-sm">
                            <input type="checkbox" name="visible_to_housekeeper" value="1"
                                class="rounded border-gray-300 dark:border-gray-700" @checked((bool) old('visible_to_housekeeper', $pivot->visible_to_housekeeper))>
                            Housekeeper can view
                        </label>
                    </div>
                </div>

                {{-- Right: media panel (modern dropzone) --}}
                <div class="bg-gray-50 dark:bg-gray-900/40 rounded-xl p-4">
                    ...
                    @if ($task->media->count())
                        <div class="mt-4">
                            <div class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-2">Existing
                            </div>
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                @foreach ($task->media as $m)
                                    <div
                                        class="rounded-lg overflow-hidden border dark:border-gray-800 bg-white dark:bg-gray-900">
                                        @if ($m->type === 'image')
                                            <img src="{{ $m->thumbnail ?? $m->url }}" alt="{{ $m->caption }}"
                                                class="w-full h-36 object-cover" />
                                        @else
                                            <video src="{{ $m->url }}" class="w-full h-36 object-cover" controls
                                                muted></video>
                                        @endif
                                        <div
                                            class="p-2 border-t dark:border-gray-800 text-xs flex items-center justify-between">
            ...
            {{-- Footer actions: Save/Cancel (only) --}}
            <div class="flex gap-2">
                <x-button type="submit">Save Changes</x-button>
                <x-button variant="secondary"
                    href="{{ route('properties.tasks.index', [$property, $room]) }}">Cancel</x-button>

                {{-- The Detach button is NOT a form here. It points to the external #detach-form --}}
                <x-button class="ml-auto bg-rose-600 hover:bg-rose-700 focus:ring-rose-500" form="detach-form"
                    type="submit">
                    Detach Task
                </x-button>
            </div>
    ...
    {{-- EXTERNAL MEDIA UPLOAD FORM --}}
    <form id="media-upload-form" action="{{ route('properties.tasks.media.store', $task) }}" method="post"
        enctype="multipart/form-data" class="hidden">
        @csrf
    </form>

    {{-- EXTERNAL DETACH FORM (separate, so Save never triggers it) --}}
    <form id="detach-form" method="post" action="{{ route('properties.tasks.detach', [$property, $room, $task]) }}"
        class="hidden">
        @csrf
        @method('DELETE')
    </form>
