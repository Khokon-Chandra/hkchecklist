...
<div
  x-data="mediaDropzone({ formId: @js($formId), accept: @js($accept), multiple: @js($multiple) })"
  x-init="
    // Augment the dropzone with a 'replace' method so each card can update itself.
...
>
  {{-- Dropzone --}}
  <div
    class="relative flex flex-col items-center justify-center rounded-2xl border-2 border-dashed p-8
  ...
  >
    <div class="pointer-events-none absolute inset-0 rounded-2xl" :class="hover ? 'ring-2 ring-indigo-400/60' : ''"></div>
    ...
    <div class="mt-3 text-center">
      <p class="text-sm text-gray-800 dark:text-gray-100">
        ...
        <button type="button" class="text-indigo-600 dark:text-indigo-400 underline font-medium" @click="pick()">browse</button>
      </p>
  ...
  <template x-if="previews.length">
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
      <template x-for="(p, i) in previews" :key="i">
        <div class="group relative rounded-xl overflow-hidden border dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">

          {{-- Full-card click overlay → opens per-card file input to replace --}}
          <input type="file"
                 class="absolute inset-0 z-20 opacity-0 cursor-pointer"
                 accept="{{ $accept }}"
                 @change="replace(i, $event)"
          ...
          <template x-if="p.kind === 'video'">
            <div class="w-full h-44 bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
              <svg class="h-10 w-10 opacity-70" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
            </div>
          </template>

          {{-- Top-right chips (visible on hover) --}}
          <div class="absolute top-2 right-2 z-10 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition">
            <span class="text-[11px] px-2 py-0.5 rounded bg-gray-900/80 text-white"
                  x-text="p.kind === 'video' ? 'Video' : 'Image'"></span>
            <button type="button"
                    class="text-[11px] px-2 py-0.5 rounded bg-gray-900/80 text-white"
                    @click.stop="remove(i)">
              Remove
            </button>
          </div>
          ...
          {{-- Hint footer --}}
          <div class="absolute bottom-10 left-0 right-0 mx-2 hidden group-hover:flex items-center justify-center">
            <span class="text-[11px] px-2 py-0.5 rounded bg-gray-900/75 text-white">Click card to replace</span>
          </div>
        </div>
      </template>
    </div>
